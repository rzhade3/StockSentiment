...
def mainLogic():
	...
	# This variable keeps track of how many articles there are 
	numArticles = 0
	for source in SOURCES_LIST:
		data = getPage(HEADLINES_URL % (source, API_KEY))
		numArticles += len(data['articles'])
	...
	averageSentiment = totalSentiment / numArticles
	print("Average Sentiment = " + str(averageSentiment))
	print(mostPositive)
	print(mostNegative)
	with open(data_file, 'r') as f:
	    json_data = json.load(f)
	json_data[time.strftime("%m/%d/%Y %H:%M")] = averageSentiment
